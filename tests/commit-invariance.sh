...
printf '2026-01-01T00:00:00Z type=dispatch job=old-job before=%s after=%s\n' \
  'tree-v2:old-scope' 'tree-v2:old-scope' > "$LOG"
O3=$(run "$D/m" 'unset MAESTRO_LOCK_TOKEN; write_lock_acquire job-one >/dev/null; write_lock_release')
[ "$(gaps "$O3")" -eq 0 ] || { echo "FAIL(3a): old v2 record compared as unequal instead of no-observation"; exit 1; }
grep -q 'after=tree-v3:' "$LOG" || { echo "FAIL(3b): no self-describing v3 baseline written"
  echo "   log: $(tail -1 "$LOG")"; exit 1; }
O4=$(run "$D/m" 'unset MAESTRO_LOCK_TOKEN; write_lock_acquire job-two >/dev/null; write_lock_release')
[ "$(gaps "$O4")" -eq 0 ] || { echo "FAIL(3c): gap on an unchanged tree once a v3 baseline exists"; exit 1; }
printf 'a\nb\n' > "$D/m/s.sh"
...
case "$B" in tree-v3:*) ;; *) echo "FAIL(4b): digest is not self-describing: $B"; exit 1 ;; esac
